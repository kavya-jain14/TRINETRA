...
export class HttpPaymentProviderAdapter implements PaymentProviderAdapter {
  ...
  constructor(sandboxUrl: string = 'http://localhost:3002') {
    this.#sandboxUrl = sandboxUrl;
  }

  async submit(input: ProviderSubmissionInput): Promise<ProviderOperationResult> {
    const response = await fetch(`${this.#sandboxUrl}/v1/simulate`, {
      method: 'POST',
    ...
    if (!response.ok) {
      throw new Error(`Sandbox returned error ${response.status}`);
    }
    const body = (await response.json()) as Record<string, unknown>;
    const providerEvent = body.provider_event as Record<string, unknown>;
    return {
      providerStatus: providerEvent.status as ProviderPaymentStatus,
      responseCode: input.scenario === 'TIMEOUT_UNKNOWN' ? 'TIMEOUT_UNKNOWN' : 'SYNTHETIC_ACK',
      providerReference: input.requestReference,
      evidence: { synthetic_scenario: input.scenario },
    };
  ...
  async inquire(input: ProviderInquiryInput): Promise<ProviderOperationResult> {
    const response = await fetch(`${this.#sandboxUrl}/v1/inquire`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    const body = (await response.json()) as Record<string, unknown>;
    return {
      providerStatus: body.providerStatus as ProviderPaymentStatus,
